...
public class MainActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        switch(item.getItemId()){
            ...
            case R.id.ai:
                ...
                Piece mover = chessBoard.grid[xfrom][yfrom].getPiece();
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        inflater.inflate(R.menu.game_options_menu_xml,menu);
        return true;
    }
    ...
    private void placePieces() {
        for (int i=0; i<8; i++){
            for (int j=0; j<8; j++){
                Piece currPiece = chessBoard.grid[i][j].getPiece();
                if (currPiece != null) {
    ...
    private boolean noValidMoves(Spot selectedSpot, Piece piece){
        int currColor = whiteTurn ? 0 : 1;
        for (int i=0; i<8; i++){
            for (int j=0; j<8; j++){
    ...
    public boolean canBlockCheck(Spot currSpot){
        ...
        for (int a=0;a<8;a++) {
            for (int b=0;b<8;b++) {
                ...
                Piece newPositionPiece=newPosition.getPiece();
                Piece current = currSpot.getPiece();
    ...
    private int getResource(Piece piece){
